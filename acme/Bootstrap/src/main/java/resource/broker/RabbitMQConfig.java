...
public class RabbitMQConfig {
    ...
    @Bean
    Binding binding_bootstrap_product(DirectExchange exchange, Queue queue_main) {
        return BindingBuilder.bind(queue_main).to(exchange).with(BOOTSTRAP_PRODUCT);
    }

    @Bean
    Binding binding_bootstrap_review(DirectExchange exchange, Queue queue_main) {
        return BindingBuilder.bind(queue_main).to(exchange).with(BOOTSTRAP_REVIEW);
    }
